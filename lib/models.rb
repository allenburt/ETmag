...
class Page < FileModel
  ...
  def date(format = nil)
    markup =~ /^\s*-# pub-date\s+(.*)/
    DateTime.parse(Regexp.last_match(1))
  end
  
  def description
    markup =~ /^\s*-# description\s+(.*)/
    Regexp.last_match(1)
  end
  
  def newer
    i = Page.find_articles.index(self)
    (i < (Page.find_articles.size - 1)) ? Page.find_articles[i+1] : nil
  end
  
  def older
    i = Page.find_articles.index(self)
    (i > 0) ? Page.find_articles[i-1] : nil
  end
